...
popular_news = "https://www.newsnow.co.uk/h/Technology/Cyber+Security?type=ts"
...
def get_news_times():
    ...
    list_req = requests.get(popular_news, headers=headers)
    if list_req.status_code == requests.codes.ok:
        soup = BeautifulSoup(list_req.content, HTML_PARSER)
    
    time_list =[]
    times = soup.find_all('span', 'time',limit=8)
    for time in times:
...
def get_news_link(j):
    ...
    top_divs = soup.find_all(href =re.compile("^https://c.newsnow.co.uk/A/"),limit=8)
